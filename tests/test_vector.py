...
def test_insert_remove():
    ...
    last_state = algo.states[-1]
    assert last_state.state_type == 'vector_insert'
    assert last_state.state_def["element"] == 12
    assert last_state.state_def["index"] == 5
    ...
    assert arr.body == [1,2,3,5,2,12]
    last_state = algo.states[-1]
    assert last_state.state_type == 'vector_remove'
    assert last_state.state_def["index"] == 1
...
def test_indexing():
    ...
    assert last_state.state_def["body"] == arr.body[1:3]
...
def test_compare():
    ...
    last_state = algo.states[-1]
    assert last_state.state_type == 'vector_compare'
    assert last_state.state_def["index1"] == 0
    assert last_state.state_def["index2"] == 1
...
def test_swap():
    ...
    assert last_state.state_type == 'vector_swap'
    assert last_state.state_def["index1"] == 0
    assert last_state.state_def["index2"] == 1
    assert last_state.state_def["body"] == arr.body
