...
def test_indexing():
    ...
    assert last_state.state_type == 'array_iter'
    assert last_state.state_def["index"] == 1

    assert arr[1,1] == arr.body[1,1]
    last_state = algo.states[-1]
    assert last_state.state_type == 'array_iter'
    assert last_state.state_def["index"] == (1,1)

    assert arr[1][2].tolist() == arr.body[1][2]
    last_state = algo.states[-1]
    second_last_state = algo.states[-2]
    assert second_last_state.state_type == 'array_declare'
    assert last_state.state_type == 'array_iter'
    assert last_state.state_def["index"] == 2

    subarr = arr[1]
    assert isinstance(subarr,ARgorithmToolkit.Array)
    last_state = algo.states[-1]
    assert last_state.state_type == 'array_declare'
    assert last_state.state_def["variable_name"] == 'arr_sub'
    assert np.all(last_state.state_def["body"] == arr.body[1])
    ...
    assert last_state.state_type == 'array_declare'
    assert last_state.state_def["variable_name"] == 'arr_sub'
    assert np.all(last_state.state_def["body"] == arr.body[1:2])
...
def test_compare():
    ...
    last_state = algo.states[-1]
    assert last_state.state_type == 'array_compare'
    assert last_state.state_def["index1"] == (0,1)
    assert last_state.state_def["index2"] == (1,1)
...
def test_swap():
    ...
    assert last_state.state_type == 'array_swap'
    assert last_state.state_def["index1"] == (0,2)
    assert last_state.state_def["index2"] == (1,2)
    assert np.all(last_state.state_def["body"] == arr.body)
...
def test_dimension_check():
    ...
    second_last_state = algo.states[-2]
    assert last_state.state_type == 'array_declare'
    assert second_last_state.state_type == 'array_iter'
    assert second_last_state.state_def["index"] == 1
    try:
