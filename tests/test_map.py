...
def test_map():
    ...
    map1.set(2,"hello")
    map1[str1] = 456
    assert algo.states[-1].state_type == "map_set"
    assert map1[str1] == 456
    assert map1["abcd"] == 123
    assert map1[2] == "hello"
    assert algo.states[-1].state_type == "map_get"
    assert map1[str1] == 456
...
def test_remove():
    ...
    map1.remove("abcd")
    assert algo.states[-1].state_def['value'] == "none"
    del map1[2]
    assert algo.states[-1].state_type == "map_remove"
    map1.get(2, "not found")
    assert algo.states[-1].state_def['value'] == "not found"
    print(map1)
